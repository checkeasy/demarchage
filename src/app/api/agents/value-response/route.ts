...
// POST: Generate a value-first response to a positive reply
export async function POST(request: NextRequest) {
  try {
    ...
    const { prospectId, replyText, replyAnalysis } = body;
    ...
    const result = await orchestrator.generateValueResponse(
      workspaceId,
      prospectId,
      replyText.trim(),
      replyAnalysis || {}
    ...
    console.error('[Agents] Value response error:', error);
    ...
    return NextResponse.json(
      { error: 'Erreur interne du serveur lors de la generation de la reponse de valeur' },
      { status: 500 }
    );
  }
}
